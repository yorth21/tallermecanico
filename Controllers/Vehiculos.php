<?php
    ...
    class Vehiculos extends Controller
    {
        ...
        public function formularioEditarVehiculo($placa)
        {
            $this->session();
            $data = $this->model->getVehiculo($placa);
            $this->views->getView($this, "editVehiculos", $data);
        }
        ...
        public function listarVehiculos()
        {
            ...
            for ($i=0; $i < count($data); $i++) {
                $data[$i]['acciones'] = '<div>
                                            <a class="btn btn-warning" href="'.base_url.'Vehiculos/formularioEditarVehiculo/'.$data[$i]['placa'].'" title="Editar"><i class="fas fa-edit"></i></a>
        ...
        public function editarVehiculo(string $placa)
        {
            $this->session();
            ...
            if (empty($propietario) || empty($placa) || empty($modelo) || empty($marca) || empty($color) || empty($tipovehiculo)) {
                ...
                if (!empty($data)) {
                    // enviar datos al modelo para ingresar el vehiculo
                    $data = $this->model->editarVehiculo($placa, $modelo, $marca, $color, $tipovehiculo, $observaciones);
                    if ($data == "ok") {
                        $msg = "ok";
                    } else {
                        $msg = "No se registro el vehiculo";
